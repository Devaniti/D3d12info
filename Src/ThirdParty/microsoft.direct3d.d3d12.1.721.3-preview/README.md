...
- `D3D12Events.h` header is now published in the AgilitySDK NuGet package (and in github.com/microsoft/DirectX-Headers).
- D3D12StateObjectCompiler.exe now supports shader cache registration with the `register`, `list-registrations`, and `clear-registrations` commands.
- d3dconfig.exe `precompiled-db-path`: overrides D3D Shader Cache Registration with the `APPLICATION_DESC` embedded in the specified PSDB.
- `ConvertLinearAlgebraMatrix` is allowed on `COMPUTE` command lists.
- Update MLIR programs for versioning and binding.
...
- DX Dump Files
  - Always generate DRED in the DX dump file creation path irrespective of the kernel device execution state.
  - Fix a crash during dump file creation caused by a dangling raw root signature pointer.

### Version 1.721.1:

- DX Dump Files fixes:
  - Honor `ID3D12DeviceConfiguration1::ConfigureDumpFile` settings — dump file configuration is now retained and passed to the driver instead of being reset to defaults.
  - Do not delete the dump file before invoking the end callback, so callers can still consume the file from the callback.
  - Renamed the dump-file feature-support field to `SupportedByOS` in `D3D12_FEATURE_DATA_DUMP_FILES_INFO` (preview header change).
- Partial and generic programs in a COLLECTION now require referenced shaders to have a root signature at collection-create time.

### Version 1.721.0:
- GPU-based validation support for generic programs, raytracing, and work graphs
- GUID texture layout
...
- Shader model 6.10 preview:
  - Linear Algebra (LinAlg) Matrix APIs
  - ClusterID
  - Triangle object positions
  - DebugBreak
  - GroupWaveIndex/GroupWaveCount
  - Expanded groupshared size
- Batched and asynchronous commands
  - Revised versions of existing clear/copy/resolve APIs that accept multiple operations in a single API
  - Updated barrier sync semantics explicitly enabling parallelism
- MLIR programs

### Version 1.719.2:

- Same as 1.719.1 plus 1.619.2 additions
